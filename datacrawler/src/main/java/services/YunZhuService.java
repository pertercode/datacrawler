...
public class YunZhuService {
        ...
        if (responseWrap.isSuccess()) {
            try {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    log.i("1分类入库 , 名称 =  " + categoryLevel1.getC_name());
                    ...
                    for (int j = 0; j < size2; j++) {
                        ...
                        log.i("2分类入库 , 名称 =  " + categoryLevel2.getC_name() + " ,  parent = " + categoryLevel1.getC_name());
        ...
        if (responseWrap.isSuccess()) {
            try {
                ...
                for (int i = 0; i < produceElements.size(); i++) {
                    ...
                    ProduceInfo produceInfo = new ProduceInfo(IDUtils.genId(platform, p_id), page, totalPage, p_id, pName, pUrl, price, imgSrc);
                    produceInfo.setpCUrl(pUrl);
                    produceInfo.setpUrl(pUrl);
    ...
    public CompanyInfo requestCompany(String url) {
        ...
        if (responseWrap.isSuccess()) {
            try {
                Document doc = Jsoup.parse(responseWrap.body, BASE_URL);
                String href = doc.select(".site-menu li").get(2).select("a").get(0).attr("href");
                ...
                for (int i = 0; i < p.size(); i++) {
                    String text = p.get(i).child(0).text().trim();
                    if (i == 0) {
                        cContactName = text;
                    }
            ...
            try {
                ...
                if (elements.size() > 0)
                    for (Element e : elements) {
                        ...
                        String t_id = IDUtils.genId(platform, IDUtils.uuid());
                        ...
                        Elements valueElements = e.select("ul.conditions-list.product-property li");
                        for (int i = 1; i < valueElements.size(); i++) {
                            ...
                            //值的Id
                            String vId = vHref.substring(vHref.lastIndexOf('/') + 1);
                            ...
                            log.i("规格的值入库了 , 名称 =  " + typeValue.gettValue() + "\t所属名称是：" + typeName.gettName());
                        }
                    }
     ...
     * 查询公司详细地址
     */
    public String requestAddress(CompanyInfo companyInfo) {
        String address = companyInfo.getcAddress();

        String cUrl = companyInfo.getcUrl();

        Request request = new Request.Builder().url(cUrl).headers(HttpUtils.getCommonHeaders()).build();
        ...
        if (responseWrap.isSuccess()) {
            try {
                Document doc = Jsoup.parse(responseWrap.body);
                Elements elements = doc.select(".enterprise.mt10 p");

                if (elements.size() > 0) {
                    address = elements.get(elements.size() - 1).text().trim();
